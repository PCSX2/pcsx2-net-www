...
summary: "Vulkanic progress and metamorphosis to Qt."
draft: false
tags:
  - "progress-report"
...
So finally we are back to the first progress report of the year at the end of 2022. Sorry for the delay but let's bulldoze to the following changes below. At this time we are at the crossroads to finally cast away the outdated WX-widgets GUI toolkit and going forwards with the Qt GUI.

x69image

At this time Vulkan was added and texture replacements are possible and countless other noteworthy changes.

The balance of writing something technical, correct and more importantly understood by many and giving it a more human feeling is hopefully noticed and appreciated.

As with many things in life where stillness is death, advancements is the only thing keeping it in life such as the Core Team, Contributors, Discord users, Wiki users, Forum users, Testers and others that aren't mentioned keep PCSX2 fresh and better with each day (maybe for some a bit too overeager) for many years now.

Enough with me mumbling about this pajazz.
I will talk about only a few improvements in details below.
...
Running blocks of like 3 cycles is very inefficient when you factor in entering and exiting a recompiler, so this makes sure at least a more reasonable amount of cycles is run. Combined with Pull Request 5571 it can give 10-20% improvement in a bunch of games.
...
Changing the maximum 3000 ms to 200 ms as the computers that can run PCSX2 don't need so much potential latency and also 1% emulation speed is changed to 2% due to multiple issues.
...
Besides EyeToy, there are other games needing a certain webcam like police 24/7.
