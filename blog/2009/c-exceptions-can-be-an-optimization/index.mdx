...
description:
  ...
  to PCSX2 was to improve the emulator's overall stability and error handling
draft: false
tags:
  - devblog
...
panic. Chants of _"C++ is slow!"_ or _"Exception handling is slow!"_
...
yet, clever use of C++ and its exceptions can actually be a _speedup_ .
...
The above code snippet must perform no less than _two conditional checks
per loop_ just to propagate the error code up the chain of function
...
The above C++ snippet performs the exact same operation, except now _no
conditionals are needed_ . We've traded off the two conditionals per
...
certainly be a _significant_ speedup over the plain jane C version. And
...
_Edit:_ I should add that the basic theory of optimization I'm using
