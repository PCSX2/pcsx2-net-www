...
PCSX2 team member and graphics expert LightningTerror has implemented per-pixel [alpha blending](https://en.wikipedia.org/wiki/Alpha_compositing) (PABE), improving both accuracy and speed. Software blending, run on the CPU, is highly accurate but incurs a performance hit as it can't take advantage of the parallelization inherent in graphics work. Hardware blending, run on the GPU, is highly efficient for this reason but can suffer in accuracy if extreme care isn't taken. Eliminating the need to run blending on the CPU, PABE blends (heh) the accuracy from software blending with the efficiency of hardware blending. The benchmarks below provided by KamFretoZ and JordanTheToast show the considerable speedup PABE brings to both AMD and Nvidia. But that wasn't the only improvement!

At the same time, he expanded PCSX2's multipass rendering to improve Direct3D11 and 12's default blending accuracy. This approach takes the benefits of [copying framebuffer] from software blending and, by tactically moving it to the GPU, [............]
...
[STILL A MESS; WORK-IN-PROGRESS] Capping off 2.2 is improved wheel input
support, thanks to the meticulous work of first-time contributor badfontkeming.
The subject of this improvement, [force
feedback](https://en.wikipedia.org/wiki/Haptic_technology#Force_feedback) (FFB),
is a type of haptic feedback, often used in racing games to simulate the
movement of the steering wheel when performing actions like cornering. These
games usually use a constant force

In games with wheel support, such as [_Gran Turismo 4_](https://wiki.pcsx2.net/Gran_Turismo_4), modern wheels would often stop producing constant FFB if they hit a timeout and wouldn't restart again until the wheel returned to neutral.

feel unresponsive or lack proper feedback due to incomplete or broken FFB support in older PCSX2 builds. This new implementation introduces more accurate FFB handling and restores the feeling of physical interaction — improving immersion and playability for racing fans.
...
- [#12069](https://github.com/PCSX2/pcsx2/pull/12069) – Fobes managed to fix the EE Cache not crash when changing from inopertrtper to teh recomplier
- [#12302](https://github.com/PCSX2/pcsx2/pull/12302) – thanks to TJnotJT they added debug UI options for new GS dumping options
