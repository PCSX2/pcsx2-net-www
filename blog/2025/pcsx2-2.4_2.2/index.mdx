...
Almost immediately after 2.0, peripherals contributor Florin9doi and first-time contributor joestringer got to work chipping away at the list of peripherals we haven't implemented yet. Thanks to them, in just about the first month of 2.0, we now had support for

These are:

- [*Rez* Trance Vibrator](https://www.theverge.com/2016/9/25/13046770/rez-trance-vibrator-ps2)
- [Gametrak/*RealPlay*](https://en.wikipedia.org/wiki/Gametrak) device
...
On top of that, PCSX2's [EyeToy](https://en.wikipedia.org/wiki/EyeToy) emulation now has audio support!
...
After 2.0, longtime contributor AirGamer launched into an effort to fix our [DEV9](https://www.psdevwiki.com/ps2/DEV9C) emulation and finally get [PS2 Linux](https://en.wikipedia.org/wiki/Linux_for_PlayStation_2) and [PlayStation Broadband Navigator](https://en.wikipedia.org/wiki/PlayStation_Broadband_Navigator) (PSBBN) working on PCSX2. His work has brought us tantalizingly close: as best we can tell, our lack of full emulation for the PS2's [translation lookaside buffer](https://en.wikipedia.org/wiki/Translation_lookaside_buffer) (TLB) is now the *only* barrier still preventing PS2 Linux and PSBBN from working. Whoever steps up and conquers the TLB will bring the penguin to PCSX2.

<PCSX2PRLink authors="badfontkeming" prNums="11906,11915">
  ### Wheel Force Feedback Improvements (2)
</PCSX2PRLink>

Capping off 2.2 is better support for wheel input, this time by newcomer badfontkeming. The subject of this improvement, [force feedback](https://en.wikipedia.org/wiki/Haptic_technology#Force_feedback) (FFB), is a type of haptic feedback, often used in racing games to simulate the movement of the steering wheel when performing actions like cornering.

In games with wheel support, such as [*Gran Turismo 4*](https://wiki.pcsx2.net/Gran_Turismo_4), modern wheels would often

### Various Smaller Improvements (4)
...
* [#11614](https://github.com/PCSX2/pcsx2/pull/11614) – Peripherals contributor Florin9doi creates a custom UI for [*Buzz!*](https://en.wikipedia.org/wiki/Buzz!) devices.
...
* [#11839](https://github.com/PCSX2/pcsx2/pull/11839) – Graphics expert LightningTerror fixes a longstanding graphical issue in [*Knights of the Template: Infernal Crusade*](https://wiki.pcsx2.net/Knights_of_the_Temple:_Infernal_Crusade).
...
The PlayStation 2 uses a [real-time clock](https://en.wikipedia.org/wiki/Real-time_clock) (RTC) to keep track of the date and time in the real world. Games often use the RTC in order to seed pseudorandom number generation (RNG) because of its high variability. Additionally, save files show a timestamp in the BIOS, and these timestamps are sometimes shown in-game too. However, some games use the RTC in a more interactive way. [*Ratchet & Clank 2*](https://wiki.pcsx2.net/Ratchet_%26_Clank:_Going_Commando) and [*Ratchet & Clank 3*](https://wiki.pcsx2.net/Ratchet_%26_Clank:_Up_Your_Arsenal), for example, let you teleport to the post-game Insomniac Museum area from 03:00–04:00, bypassing completion checks. [*The Simpsons: Hit & Run*](https://wiki.pcsx2.net/The_Simpsons:_Hit_%26_Run), [*The Simpsons: Road Rage*](https://wiki.pcsx2.net/The_Simpsons:_Road_Rage), [*SpongeBob SquarePants: Battle for Bikini Bottom*](https://wiki.pcsx2.net/SpongeBob_SquarePants:_Battle_for_Bikini_Bottom), and [*Scooby-Doo! Night of 100 Frights*](https://wiki.pcsx2.net/Scooby-Doo!_Night_of_100_Frights) have holiday-exclusive Easter eggs. [*Need for Speed: Hot Pursuit 2*](https://wiki.pcsx2.net/Need_for_Speed:_Hot_Pursuit_2) displays the date and time on a camera recording when you get busted. And [*Metal Gear Solid 3*](https://wiki.pcsx2.net/Metal_Gear_Solid_3:_Snake_Eater)... if you know, you know.

By default, when a user starts a game, PCSX2 sets the RTC to the date and time from the user's desktop. For input recordings which require perfect determinacy to function correctly, we instead used a fixed, arbitrary date and time after the release of *Metal Gear Solid 3* (which crashes if the date is too old). With the introduction of RTC settings by TheTechnician27, you can now optionally select a custom RTC starting point in the per-game settings (`Game Properties` > `Emulation` > `Real-Time Clock`). This provides niche benefits for regular users (especially *Ratchet* players and enjoyers of Hideo Kojima's antics), and it brings PCSX2 closer to having a mature suite of [tool-assisted speedrun](https://en.wikipedia.org/wiki/Tool-assisted_speedrun) (TAS) capabilities.
...
<PCSX2PRLink authors="refractionpcsx2,JordanTheToaster" prNums="12350,12636,12645,12705">
  ### New Upscaling Fix (4)
</PCSX2PRLink>

Thanks again to former PCSX2 lead developer refraction, PCSX2 now has a brand-new tool in its arsenal to fix misalignment issues caused by upscaling. Based on the existing Align to Native (AtN) half-pixel offset (HPO) option, Align to Native with Texture Offset (AtNTO) often combines the best of AtN and another HPO option called Special (Texture) to make a lot of games much sharper when upscaled. Thanks to a massive effort by GameDB specialist JordanTheToast, PCSX2 now applies this new setting automatically to over 100 games. These include [*Ace Combat 04: Shattered Skies*](https://wiki.pcsx2.net/Ace_Combat_04:_Shattered_Skies), the [*Sly Cooper*](https://wiki.pcsx2.net/Sly_Cooper) trilogy, [*God of War*](https://wiki.pcsx2.net/God_of_War) and [*God of War II*](https://wiki.pcsx2.net/God_of_War_II), and [*Shadow of the Colossus*](https://wiki.pcsx2.net/Shadow_of_the_Colossus). Below is a before and after of *Ace Combat 04*.
...
Below are benchmarks comparing PCSX2 2.0 to PCSX2 2.4, keeping in mind that since then, we've become even more accurate – especially when it comes to eliminating upscaling-related artifacting.
...
This brings massive fixes to a huge variety of games – at minimum, multiple games with two-player splitscreen (such as [*Jak X: Combat Racing*](https://wiki.pcsx2.net/Jak_X:_Combat_Racing)), multiple games that no longer need a hack to work (such as [*Ghost in the Shell: Stand Alone Complex*](https://wiki.pcsx2.net/Ghost_in_the_Shell:_Stand_Alone_Complex)), multiple games whose effects now work (such as [*Drakengard*](https://wiki.pcsx2.net/Drakengard); comparison below), and multiple games that would only work before in software rendering (such as [*Hitman: Contracts*](https://wiki.pcsx2.net/Hitman:_Contracts); comparison below).
