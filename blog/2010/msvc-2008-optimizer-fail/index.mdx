...
**_templated code_** and **_inlined functions_** .
...
_Edit/Update: This bug only appears to manifest itself when the input
...
_ignored_ , Visual C++ will still generate code that assumes the
...
calling function ( _void main()_ in this case)will have to flush/reload
...
alignment, _even if the actual SSE code portions are optimized away_ .
...
function to use _only_ the selected path. If _'value'_ is 1, it runs
...
_sparse_ switches. A sparse switch is one where the values are not
contigious. Example:
...
In this example, MSVC's optimizer will employ the use of a _binary
search_ to dispatch the switch. Rather than compare each value
...
optimization and is often times _faster_ than using function lookup
tables.

... but it actually _backfires_ if the **toggle** value is a known
constant (such as a template parameter). The optimization method of the
switch statement is made by MSVC 2008 _before_ it eliminates unused
