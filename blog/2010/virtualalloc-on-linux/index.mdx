...
description:
  Yes, there is a way to simulate Microsoft's VirtualAlloc behavior on
  Linux
draft: false
tags:
  - devblog
...
memory simply by specifying PROT*NONE on any anonymous mapping (in the
...
\_commit* the memory (equivalent to VirtualAlloc with MEM_COMMIT). If
...
programs _commit_ a lot more RAM than is actually available to the
...
ram only use a wee bit of it, so that's fine. By using _overcommitted_
...
will also work _well_ on Linux systems that have over-commit disabled,
