...
ADD dest_reg, src1_reg, src2_reg // Add source1 and source2 registers,
and store the result in destination register\
BR relative_address // Branch (jump) to relative address (PC +=
relative_address \* 4)\
\
...
also need to set rec_cache to the new x86 emitter address (so it can
