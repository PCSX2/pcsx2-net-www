...
this type of blending is impossible on the fixed function unit of a
PC\'s GPU.
...
GPU\'s fragment shader. Fragment shaders are very small dedicated CPUs
so it is quite easy to do a few small operations on them.
...
width="20" height="20"} The end result is that we can implement basic
blending in the fragment shader instead of relying on the limited GPU
core.\
\
