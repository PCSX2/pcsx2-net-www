    ...
    <tr>
      ...
      <td>
        ...
        <ul>
          ...
          <li>`beshort`: Two bytes, to be stored in big-endian order in memory (most significant byte first).</li>
          <li>`beword`: Four bytes, to be stored in big-endian order in memory (most significant byte first).</li>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td rowspan="2">
        ...
        <ul>
          <li>`0`: Equal (skip if not equal)</li>
          <li>`1`: Not equal (skip if equal)</li>
          <li>`2`: Less than</li>
...
The psuedocode in the above table makes use of the following functions: `Read(address, size)` which returns `size` bits read from memory at `address`, and `Write(address, size, value)` which writes `value`, which is `size` bits in size, to memory at `address`.
